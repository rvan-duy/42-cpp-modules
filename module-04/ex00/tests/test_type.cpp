...
SCENARIO("Initializing Animal class and testing methods", "[Animal Class]") {
    GIVEN("Animal Class") {
        WHEN("Animal Class is an Animal") {
            ...
            THEN("Animal type is empty") {
                REQUIRE(animal->getType() == "");
            };
            delete animal;
        };
        WHEN("Animal Class is a Dog") {
            Animal * dog = new Dog();
            THEN("Animal type is Dog") {
                REQUIRE(dog->getType() == "Dog");
            };
            delete dog;
        };
        WHEN("Animal Class is a Cat") {
            Animal * cat = new Cat();
            THEN("Animal type is Cat") {
                REQUIRE(cat->getType() == "Cat");
            };
            delete cat;
        };
    };
